...
In an A2A ecosystem, security is critical because agents often execute code or access paid APIs (like LLMs or data access). You don't want a random agent on the network draining your API quota. Let's extend the above sample to add a **Security Layer** using Bearer Token authentication.

The Security Flow:

1. **The Worker (Server)** mandates that every incoming request must have a valid "badge" (API Key) in the header.
2. **The Manager (Client)** attaches this "badge" to its handshake and subsequent requests.

### 1. The Secured Worker Agent (Server)

This script now includes an authentication dependency. If a request arrives without the correct token, the agent will reject it with a `401 Unauthorized error`.
```python
import os
from google.adk import Agent, Server, HTTPException
from google.adk.security import BearerAuth # Hypothetical helper for standard Auth

# --- Security Configuration ---
# NEVER hardcode secrets in your source code. 
# We load the "Secret Handshake" key from the environment.
# If this variable isn't set, we default to a placeholder for local testing,
# but in production, this should cause a hard failure.
API_SECRET = os.getenv("A2A_WORKER_SECRET", "change-me-locally")
...
async def verify_api_key(token: str):
    ...
    Args:
        token: The string extracted from the 'Authorization: Bearer <token>' header.
    """
    if token != API_SECRET:
        # If the keys don't match, we slam the door.
        # This prevents the agent from even parsing the task payload.
        raise HTTPException(
            status_code=401, 
            detail="Invalid Authentication: You are not authorized to use this Math Agent."
        )

# --- Define the Agent & Tools (Same as before) ---
def perform_complex_calculation(expression: str) -> str:
    # Security Note: Even with auth, always sanitize inputs (e.g., limit eval capabilities)
    try:
...
worker_agent = Agent(
    name="Secure-Math-Worker",
    instructions="You are a secure math assistant.",
    tools=[perform_complex_calculation]
)

# --- Initialize the Server with Security ---
# We inject the 'verify_api_key' function as a dependency.
# The Server will run this function for *every* incoming request 
# before letting the agent process it.
server = Server(
    agent=worker_agent,
    auth_dependency=BearerAuth(verify_api_key) 
)

if __name__ == "__main__":
    print(f"🔒 Secure Worker running on port 8080. Secret: {API_SECRET[:3]}***")
    server.run(host="0.0.0.0", port=8080)
```
### 2. The Authenticated Manager Agent (Client)

The Client must now present the credentials. If it forgets, the connection will be refused.
```python
import asyncio
import os
from google.adk import Agent, A2AClient
...
# The client must have the same secret key as the worker.
WORKER_SECRET = os.getenv("A2A_WORKER_SECRET", "change-me-locally")

async def main():
    manager = Agent(
        name="Project-Manager",
        instructions="Delegate tasks securely."
    )

    # --- Step 1: Secure Connection ---
    # We pass the 'auth_token' (or headers) to the client.
    # The library will automatically attach "Authorization: Bearer <secret>" 
    # to every HTTP request (GET agent.json, POST tasks, GET status).
    print(f"🔐 Connecting to worker at {WORKER_URL} with authentication...")
    
    try:
        client = A2AClient(
            base_url=WORKER_URL,
            auth_token=WORKER_SECRET # <--- The Key Addition
        )
        
        # Test the connection/auth by fetching the agent's profile
        worker_info = await client.get_agent_info()
        print(f"✅ Success! Connected to: {worker_info.name}")

    except Exception as e:
        print(f"❌ Connection Failed: {e}")
        return

    # --- Step 2: Create a Task (Authorized) ---
    task_prompt = "Calculate (50 * 50) / 10"
    
    print(f"\nSending task: '{task_prompt}'")
    
    # This request will now pass the 401 check on the server
    task_handle = await client.create_task(prompt=task_prompt)
...
### Best Practices for A2A Security

1. **Granular Permissions (Scopes)**:
    - In a production system (like OAuth2), you wouldn't just check `token == secret`. You would check scopes.
    - Example: The "Refund Agent" might accept a token from the "Manager Agent" (Scope: `read/write`), but only a "read-only" token from an "Auditor Agent" (Scope: `read`).

2. **Audit Logging**:
    - Inside the `verify_api_key` function, you should log who is calling.
    - `logger.info(f"Agent {token_identity} accessed tool {tool_name}")`.

3. **Network Isolation**:
    - For high security, don't expose the Worker Agent to the public internet. Use a Virtual Private Cloud (VPC) or a mesh network (like Istio/Envoy) so that only internal agents can even route packets to the worker.
---
